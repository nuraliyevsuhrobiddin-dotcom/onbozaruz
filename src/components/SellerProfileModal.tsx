...
export const SellerProfileModal: React.FC = () => {
  const { selectedSellerModal, setSelectedSellerModal, posts, products, followedSellerIds, toggleFollowSeller, setProductDetail, openVideoViewer, showToast, currentUser, setAuthPromptOpen } = useAgroStore();
  ...
  return <Modal isOpen onClose={() => setSelectedSellerModal(null)} title="Sotuvchi profili">
    <div className="space-y-4 pb-2">
      ...
      <div className="grid grid-cols-3 gap-2"><button onClick={() => toggleFollowSeller(sellerId, sellerName)} className={`rounded-2xl py-3 text-xs font-black ${following ? 'bg-slate-100 text-slate-700' : 'bg-[#e54b3f] text-white'}`}>{following ? <><UserCheck className="mr-1 inline h-4 w-4" />Obunadasiz</> : <><UserPlus className="mr-1 inline h-4 w-4" />Obuna</>}</button>{phone ? <a href={currentUser ? `tel:${phone.replace(/[^+\d]/g, '')}` : undefined} onClick={(e) => { if (!currentUser) { e.preventDefault(); showToast("Bog'lanish uchun avval tizimga kiring"); setSelectedSellerModal(null); setAuthPromptOpen(true); } }} className="rounded-2xl bg-slate-900 py-3 text-center text-xs font-black text-white cursor-pointer"><PhoneCall className="mr-1 inline h-4 w-4 text-emerald-400" />Aloqa</a> : <button onClick={() => showToast('Telefon raqami ko‘rsatilmagan')} className="rounded-2xl bg-slate-100 py-3 text-xs font-black text-slate-500"><MessageCircle className="mr-1 inline h-4 w-4" />Aloqa</button>}{telegram ? <a href={currentUser ? `https://t.me/${telegram}` : undefined} target={currentUser ? "_blank" : undefined} rel="noreferrer" onClick={(e) => { if (!currentUser) { e.preventDefault(); showToast("Bog'lanish uchun avval tizimga kiring"); setSelectedSellerModal(null); setAuthPromptOpen(true); } }} className="rounded-2xl bg-[#0088cc] py-3 text-center text-xs font-black text-white cursor-pointer"><Send className="mr-1 inline h-4 w-4" />Telegram</a> : <button onClick={() => showToast('Telegram manzili ko‘rsatilmagan')} className="rounded-2xl bg-slate-100 py-3 text-xs font-black text-slate-500">Telegram</button>}</div>
